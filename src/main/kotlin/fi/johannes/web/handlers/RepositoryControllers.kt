package fi.johannes.web.handlers

import com.github.salomonbrys.kodein.Kodein
import com.github.salomonbrys.kodein.bind
import com.github.salomonbrys.kodein.instance
import com.github.salomonbrys.kodein.singleton
import fi.johannes.data.services.proxy.BackupService
import fi.johannes.web.handlers.repository.RepositoryController
import fi.johannes.web.handlers.repository.RepositoryControllerImpl
import fi.johannes.web.handlers.status.StatusController
import fi.johannes.web.handlers.status.StatusControllerImpl

/**
 * Johannes on 14.1.2018.
 */
class RepositoryControllers(backupService: BackupService) {

    val injector = Kodein {
        bind<RepositoryControllerComponents>() with singleton {
            RepositoryControllerComponentsImpl(backupService)
        }
        bind<RepositoryController>() with singleton { RepositoryControllerImpl(instance()) }
        bind<StatusController>() with singleton { StatusControllerImpl(instance()) }
    }

}